...
int_fast32_t inline quiescence_search(Position* position, int_fast32_t my_best_move_score,
                                      int_fast32_t their_best_move_score, uint64_t* nodes)
{
  ...
  *nodes += 1;
  ...
  for (uint32_t move : possible_moves)
  {
    ...
    int_fast32_t current_score = -quiescence_search(&new_position, -their_best_move_score, -my_best_move_score, nodes);
                     ...
                     uint64_t* nodes)
{
  if (depth == 0)
  {
    ...
    return quiescence_search(position, my_best_move_score, their_best_move_score, nodes);
  }
